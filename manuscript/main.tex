...
to estimate light precipitation using CloudSat observations and
moderate-to-heavy precipitation using Dual-Frequency Precipitation Radar
observations. The two estimates are combined using a fusion scheme to obtain a
consistent precipitation estimate across precipitation regimes. Validation
against in situ measurements from shipborne disdrometers shows a 25\% improvement
in the detection skill for high-latitude precipitation and a reduction in the
underestimation of high-latitude and frozen precipitation by more than 50%
compared to retrievals constrained only by precipitation radar data. However,
the fused retrieval does not improve the precision of instantaneous
precipitation estimates, which is likely due to significant random errors in the
CloudSat-based reference estimates of liquid precipitation.\\
%
...
reference instrument alone. The proposed retrieval provides a pathway to augment
GPM reference precipitation estimates and improve the representation of oceanic
precipitation in future GPM precipitation products.\\
%
}
...
oceanic precipitation from three principal precipitation datasets: PMW
precipitation estimates from GPM Microwave Imager (GMI) retrieved using the V07
of the Goddard Profiling Algorithm (GPROF) retrieval algorithm, the Global
Precipitation Climatology Project (GPCP, (Huffman et al. 2023)), and the ERA5
reanalysis (Hersbach et al. 2020). While the datasets are largely consistent
across the tropics and mid-latitudes, the zonal means diverge poleward of 40 °
...
accumulations at high latitudes than in the tropics and are both harder to
simulate in models due to weaker linkages to synoptic forcings, and are harder
to estimate from satellite observations due to their weaker radiometric
signatures and larger dependence on microphysical variations. The GPM
Dual-Frequency Precipitation Radar (DPR) has limited sensitivity, causing it to
miss shallow and frozen precipitation. PMW retrieval algorithms such as GPROF are
constructed to reproduce DPR-derived reference precipitation conditioned on the
observed radiances and consequently inherit the limitations of the radar
reference.

Merged products such as IMERG and GPCP partially compensate for this deficiency
using climatological adjustments. GPCP, for example, rescales microwave
retrievals using precipitation climatologies derived in part from the CloudSat
Cloud Profiling Radar (CPR), which is more sensitive to light snowfall and
extends observations to higher latitudes. However, such corrections constrain
long-term means rather than instantaneous retrievals and may therefore
misrepresent the temporal variability of high-latitude precipitation.

\begin{figure}[h]
 \centerline{\includegraphics[width=37pc]{figures/figure01.png}}
...
Recent neural-network precipitation retrievals have improved performance across
many regimes and are being adopted operationally. However, machine-learning
retrievals approximate the conditional expectation defined by their training
data and thus reproduce biases present in the reference observations. Training
solely on DPR-based precipitation therefore propagates the radar’s limited
sensitivity to light and frozen precipitation into the learned retrieval,
regardless of retrieval technique used.

Here we aim to overcome the limitations of current CPR- and DPR-based
precipitation estimates by using passive microwave (PMW) observations to bridge
the sensitivity gap between the CPR and DPR and thus achieve more complete
...
reference data and moderate to heavy precipitation estimate based on DPR
...
the GPROF and GPROF-NN retrievals. It is part of the development of GPROF V08 --
the next operational version of the GPM PMW retrievals -- and aims to address
the systematic underestimation of high latitude precipitation. The previous
operational version of GPROF -- GPROF V07 -- used light precipitation estimates
from the MiRS algorithm to fill in missing precipitation in DPR estimates but
this was unsuccessful in fully addressing the underestimation. The filling in of
missing oceanic precipitation complements several other interventions addressed
to fix shortcomings in the DPR data used as a priori database and training
dataset such as the use of ground-based radar estimates over snow-covered
surface and the use of ERA5 precipitation over sea ice.

This manuscript describes the GPROF-NN XPR retrieval and validates its using
in-situ measurements from shipborne hydrometeors demonstrating its ability to
improve the detection of light and frozen precipitation and the reduction of
climatological biases. Additionally, we provide a validation of the CPR- and
DPR-based reference estimates against independent measurements from ground-based
precipitation radars. While the proposed retrieval constitutes a step forward
towards improving the representation of precipitation at high latitudes, it also
highlights important inconsistencies in the precipitation products currently
used to constrain oceanic precipitation.
...
provide a general precipitation estimate that better represents the full
spectrum of precipitation intensities. Below we describe the retrieval
...
introduced in Pfreundschuh (2022) and subsequently evaluated in Pfreundschuh
(2024). It uses the same neural-network architectures as the updated version of GPROF-NN that is currently under
...
composed of EfficientNet-V2–style inverted bottleneck convolutional blocks (Tan
& Le, 2021). Following the normalization and activation design adopted in
ConvNeXt (Liu et al., 2022), each convolutional block uses Layer Normalization
(Ba et al., 2016) and GELU activation functions (Hendrycks & Gimpel, 2016). The
network input consists of GMI brightness temperatures together with the
corresponding earth-incidence angles (EIA) for all channels and a set of
ancillary variables. These ancillary data describe both surface conditions (land
fraction, elevation, mountain classification, leaf-area index, ice fraction, and
snow depth) and the meteorological environment (2-m temperature, 10-m wind
speed, orographic wind, and total column water vapor).
...
regression (Pfreundschuh, 2018). The expectation is that the CPR-based reference
data will miss heavy rain where CPR is attenuated while the DPR-based reference
will miss light precipitation below the detection threshold of those radars.
...
precipitation estimates from CloudSat and DPR.

For the GMI–CloudSat dataset, precipitation estimates are derived by combining
information from the 2C-Precip-Column, 2C-Rain-Profile, and 2C-Snow-Profile
products. Precipitation rates are assigned according to the precipitation
classification in 2C-Precip-Column: pixels flagged as non-precipitating are
assigned zero precipitation; pixels classified as rain are assigned the rate
from 2C-Rain-Profile; and pixels classified as snow are assigned the rate from
...
training. The dataset includes all collocations from 2014–2017 with a maximum
...
For the GMI–DPR dataset, precipitation rates are taken from the GPM 2B-CMB
...
the GMI–CloudSat dataset, ensuring comparable learning weight for both reference
...
The GPROF-NN XPR retrieval produces, for each GMI pixel, a CPR-based and a
DPR-based precipitation estimate. To provide a single precipitation estimate
that properly describes precipitation across precipitation regimes, the two
estimates must be combined. The fused estimate relies on the CPR-based estimate
when the DPR-based retrieval indicates no precipitation (reflectivities below
the DPR threshold) and transitions to the DPR-based estimate as intensity
increases (reflectivities likely attenuated in CPR). A simple linear transition,
however, was found to introduce a notable bump in the distribution of the fused
...
CPR-based estimates at low precipitation rates and transitions follow the
...
The OceanRAIN dataset provides shipborne disdrometer measurements of
precipitation rates over the ocean. For validation of the GPROF-NN XPR
retrieval, we identify GMI overpasses of the ship locations from March 2014, the
start of GMI observations, through September 2018, prior to the period used for
GPM-DPR training collocations. Because this interval overlaps with the
GMI–CloudSat training dataset, any coincident matchups are excluded from the
evaluation.
...
minimum relative error across these three datasets exceeds 1.8 are discarded.
The quality control removes 26 samples from the 3436 collocations with valid
precipitation estimates. This removes both precipitation events missed by the
distrometer as well as cases where the distrometer indicates very high
precipitation rates not seen in the satellite or ERA5 data. However, the
exclusion of these events does not impact the qualitative results of the
...
we assess the CPR- and DPR-constrained reference precipitation estimates using
independent observations from NOAA’s Multi-Radar Multi-Sensor (MRMS) product
along the U.S. East Coast. For CloudSat, all available overpasses from June 2015
through the end of the mission are included. Owing to the wider swath of the
DPR, overpasses from the year 2022 alone provide sufficient sampling for a
robust comparison.
...
bias and mean-squared error and high linear correlation. For the CPR-based
estimates, on the other hand, the accuracy is substantially lower: MSE and
correlation coefficient indicate lower accuracy that is also reflected in the
increased spread of the retrieved values. In part, this can be explained by the
higher resolution of the CPR-based precipitation estimates. Both training and
testing data uses CPR estimates at the native resolution (1.6 km), whereas the
DPR-based estimates are downsampled to the GMI resolution, which is how they are
traditionally treated in the GPROF framework. The different approach for the
CPR-based estimates was chosen due to the difficulties of assigning a unique
resolution to the GMI observations and downsampling the one-dimensional CPR data
to the two-dimensional GMI footprints.
...
exceeding 3-4 mm/h. However, since CloudSat estimates have to be considered very
...
Figure~\ref{fig:case_study} illustrates an example of the DPR- and CPR-based precipitation estimates together with the resulting fused estimate. The case corresponds to a GMI overpass of a Southern Hemisphere mid-latitude cyclone in June 2018. A comparison of the CPR-based, DPR-based, and fused XPR precipitation fields highlights substantial structural differences among the three products. Although the CPR- and DPR-based estimates are generated using the same retrieval framework, they exhibit marked discrepancies. The CPR-based retrieval produces higher precipitation intensities and a broader spatial extent of the cyclone’s precipitation field. The fused XPR product moderates the elevated precipitation rates evident in the CPR-based retrieval while largely preserving its enhanced spatial coverage.
Comparison with the reference data along the CloudSat swath indicates that the PMW retrieval captures the main precipitation features but does not reproduce the small-scale variability present in the radar-based reference estimates. This discrepancy is expected because PMW observations have coarser spatial resolution and sampling than the radar measurements. Comparison with the CPR and DPR radar reflectivity fields further shows that the GPROF-NN XPR retrieval successfully detects the shallow precipitation occurring between 200 and 250 km along the CloudSat track. Additional regions appear around 50 km and 750 km along the swath where the PMW retrieval indicates precipitation while the radar estimates suggest little or no precipitation. However, in most of these areas the CPR radar still detects reflectivities extending down to the surface, suggesting that precipitation likely reaches the surface but may be overestimated by the PMW retrieval.
...
\begin{figure}[h]
 ...
 \caption{
   ...
   DPM DPR precipitation (a), on CloudSat CPR precipitation (b), and the fused
...
observations as well as the sensitivity ranges of the reference datasets.
However, it remains essential to evaluate the algorithm against independent
precipitation datasets. Such validation ensures that the improved agreement with
the training and reference products corresponds to a genuine enhancement in
light precipitation estimation skill that can be verified independently.
...
parameterization of Sims (2015) with ERA5-derived wet-bulb temperatures.
Overpasses with a conditional probability of solid precipitation exceeding 50\%
are categorized as likely mixed or frozen precipitation events.
...
higher correlation coefficient for all overpasses. However, when comparing the
estimates to the 1028 OceanRAIN overpasses with valid 2BCMB reference
precipitation estimates, the GPROF-NN DPR estimates achieve a MSE of 0.26 (mm
h$^{-1}$)$^2$ and a linear correlation coefficient of 0.84 compared to 0.32 (mm
h$^{-1}$)$^2$ and a linear correlation of 0.8 for the GPROF V7 baseline. This
indicates that these small accuracy differences are likely due to statistical
noise in the validation data.
...
Figure~\ref{fig:mrms_scatter} displays scatter plots of the CPR- and DPR-based precipitation
estimates compared to ground-based radar measurements. The CPR-based
precipitation estimates exhibit very high scatter compared to the MRMS-based
measurements particularly for MRMS precipitation rates up to 1 mm/h. This is
surprising given that the CloudSat estimates are expected to be more reliable at
low precipitation rates, where the radar observations are less affected by
attenuation. Nonetheless, the estimates achieve a correlation of 0.4 likely due
to the better agreement for MRMS precipitation rates exceeding 1 mm/h. The
DPR-based estimates show better agreement with the MRMS estimates. The DPR based
estimates strongly overestimate any MRMS estimates below 0.5 mm/h, which is a
result of the limited sensitivity of the DPR. Starting at 0.5 mm/h, however, the
DPR-based estimates show very good agreement with the MRMS-based estimates up to
about 10 mm/h, after which they start to exhibit a tendency towards
under-estimation.
...
validation dataset illustrating the failure modes visible in the scatter plots.
While the MRMS precipitation measurements indicate spatially homogeneous with
precipitation rates around 1 mm h$^{-1}$ throughout most of the scene, the
CPR-based precipitation oscillate between 0.01 and 10 mm h$^{-1}$. The principal
change in CPR radar reflectivities during the transition from 10 mm h$^{-1}$ to
0.01 mm h$^{-1}$ is a strong increase in the surface backscatter while the
above-surface reflectivities remain relatively similar. While limited, this
evidence suggest that the errors in the CPR-based precipitation estimates may be
related to the handling of the surface backscatter rather than inherent
...
\section{Global Precipitation Distributions}
